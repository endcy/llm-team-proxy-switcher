...
| 痛点 | 本项目的解决方案 |
|------|----------------|
| ❌ 单个 API Key 频繁触发 429 限流 | ✅ 多个 Key 自动轮询，叠加团队配额 |
...
这正是本项目要做的事：**一个 JSON 配置文件 + 一个 JS 文件 + 一条启动命令**，即可让整个团队共享所有成员的 Coding Plan 配额，429 限流时自动无缝切换。
...
- 直接配置：只能用固定的一个 key + 一个 model，429 了就直接报错
- 通过代理：代理自动在多个 key + 多个 model 之间轮询，429 了无缝切换
...
**轮询规则：**
1. 请求首先使用目标 [0]（P0 的第一个模型）
2. 返回 429 → 目标 [0] 进入冷却，自动切换到目标 [1]
3. 目标 [1] 也 429 → 切换到目标 [2]（换 Provider + Key）
4. 依次尝试，直到找到可用的组合
5. 冷却时间到 → 目标 [0] 恢复 → 自动切回首选
...
Claude Code 的 hooks 只能拦截工具调用（PreToolUse/PostToolUse）和注入上下文，**无法拦截底层 API 传输层的 429 错误**。429 发生在 Claude Code 内部与 API 的通信层，对插件完全不可见。
