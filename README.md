...
- **401 智能重试** — API Key 失效时自动重试 5 次，确认失效后切换到下一个 Key
...
这正是本项目要做的事：**一个 JSON 配置文件 + 一个 JS 文件 + 一条启动命令**，即可让整个团队共享所有成员的 Coding Plan 配额，429 限流 / 524 超时 / 529 服务繁忙 / 401 认证失败时自动无缝切换。
...
- 直接配置：只能用固定的一个 key + 一个 model，429 / 524 / 529 / 401 了就直接报错
- 通过代理：代理自动在多个 key + 多个 model 之间轮询，429 / 524 / 529 了无缝切换，401 自动重试 5 次后切换
...
**情况 C：认证失败（401 Unauthorized）**
- 代理自动重试当前 Key（最多 5 次，排除临时故障）
- 5 次后仍 401，判定 Key 失效，切换到下一个 Key
- 请求成功后自动重置 401 计数
...
- **新增 Codex CLI 支持**：
  - 支持 Codex 0.130+ 的 Responses API（`/responses` 端点）
  - 自动将 Responses API 请求转换为 Chat Completions 格式，兼容所有 upstream provider
  - 自动将 `developer` 角色映射为 `system`（兼容国内 provider）
  - 流式 SSE 格式完整转换（包含 `sequence_number`、完整事件序列）
  - 支持非流式和流式响应、工具调用（function_call）转换
- **新增 401 Unauthorized 智能重试**：
  - API Key 失效（401）时自动重试当前 Key 最多 5 次
  - 5 次后仍失败，自动切换到下一个 Key
  - 请求成功后自动重置 401 计数
  - 与 429/524/529 的立即切换不同，401 有重试缓冲，避免临时故障导致误切换
